...
if __name__ == '__main__':
    ...
    ### rmse case
    evaluator = Evaluator('abcd-struct') #ogbg-molipo to abcd-struct, 
    print(evaluator.expected_input_format)
    print(evaluator.expected_output_format)
    y_true = torch.tensor(np.random.randn(100,1)).view([-1, 1]) #원래는 100
    y_pred = torch.tensor(np.random.randn(100,1)).view([-1, 1]) #원래는 100
    input_dict = {'y_true': y_true, 'y_pred': y_pred}
    result = evaluator.eval(input_dict)
    print(result)
    
    ### rmse case
    evaluator = Evaluator('abcd-func')
    print(evaluator.expected_input_format)
    print(evaluator.expected_output_format)
    y_true = torch.tensor(np.random.randn(100,1)).view([-1, 1]) #원래는 100
    y_pred = torch.tensor(np.random.randn(100,1)).view([-1, 1]) #원래는 100
