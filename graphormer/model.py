...
class Graphormer(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs): #로그를 까보면 다 있다는 거야??
        ...
        if self.dataset_name == 'ogbg-molpcba':
            ...
            valid_loss = self.loss_fn(y_pred, y_true)
            try:
                self.log('valid_' + self.metric, self.evaluator.eval(input_dict)
                         [self.metric], sync_dist=True, logger=True) #valid rmse
                self.log('valid_loss', loss, sync_dist=True)
            except:
                pass
        return self.evaluator.eval(input_dict)[self.metric], valid_loss
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log('test_' + self.metric, self.evaluator.eval(input_dict)
                 [self.metric], sync_dist=True, logger=True) #test rmse
        self.log('test_loss', test_loss, sync_dist=True)
        return self.evaluator.eval(input_dict)[self.metric], test_loss
